...
export function createSubscriptionRegistry(): SubscriptionRegistry {
  const perKey = new Map<string, Set<() => void>>();
  const global = new Set<() => void>();

  const subscribeKey = (key: string, listener: () => void) => {
    ...
    return () => {
      ...
      if (current.size === 0) perKey.delete(key);
    };
  };
   ...
   */
  const notifyKeys = (changedKeys: readonly string[]) => {
    if (changedKeys.length === 0 && global.size === 0) return;
    batch(() => {
